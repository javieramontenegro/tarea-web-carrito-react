...
const Container = () => {
  ...
  const [productos, setProductos] = useState([
    { nombre: 'Producto 1', precio: 10000, cantidad: 2 },
    { nombre: 'Producto 2', precio: 5000, cantidad: 1 },
  ]);
  ...
  // Calcular el total con el costo de delivery
  const calcularTotal = () => {
    const subtotal = calcularSubtotal();
    const totalDelivery = 0; // Puedes cambiar este valor dependiendo de si hay delivery o no
  ...
  return (
    <div className="w-full max-w-[100%] p-[10px]">
      ...
      <form className="flex flex-col" action="">
        ...
        <input
          className="border border-gray-300 rounded-[5px] min-h-[30px] mb-[15px] bg-white px-[2px] py-[1px]"
          type="text"
          name="nombre"
        />
        ...
        <textarea
          className="border border-gray-300 rounded-[5px] min-h-[100px] mb-[15px] bg-white px-[2px] py-[1px]"
          name="comentarios"
          id="comentarios"
        />
        <label className="flex items-center justify-between w-auto px-[10px] py-[15px] border border-gray-200 rounded-[5px] bg-white pointer mt-[15px]">
          <div className="flex items-center font-bold">
            <div className="bg-[url(./assets/transferencia.svg)] w-[20px] h-[20px] mr-[10px] bg-no-repeat bg-center rounded-[5px] bg-size-[100%]"></div>
            Transferencia bancaria
          </div>
          <input
            className="cursor-pointer cursor-grab [accent-color:rgb(201,0,0)] absolute w-[0] h-[0]"
            type="radio"
            id="transferencia"
            name="metodo"
          />
        ...
        <div className="w-auto h-auto p-[10px] flex justify-between mt-[5px]">
          <span>Total</span>
          <span className="font-bold text-xl">${calcularTotal()}</span>
        </div>
        <button
          type="submit"
          className="w-auto bg-red-700 rounded-[5px] text-white border min-h-[50px] font-bold mt-[20px]"
        >
          Confirmar pago
        </button>
      </form>
